...
windows下可以进行测试<br>
### 下载数据集
将[VOC]()数据解压放置在data目录下,里面的SegmentationClassAug文件是DrSleep提供的，是shape为[h,w,1]每一个像素值都对应类别的label,我将它们放到了一起。<br><br>
将[restnet预训练数据]()解压放置在该模型的根目录下。<br><br>
如果需要模型预训练数据可以将我训练的[权重数据]()解压放置在model目录下。<br><br>
### 代码介绍
